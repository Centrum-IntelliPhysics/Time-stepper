...
parametric=0; %parametric DeepONet
identity=0;
...
%simmetry=1;
%
val_epsilon=PARAM_test_branch(2000); %0.1 or 1 (is effectively 3.8 max)  <--------------------
% %val_epsilon=1;
% %
if val_epsilon<1
    parametric=0;
...
ns=sum(I1);
ff_train=UV0_train_branch(:,I1);
param_train=PARAM_train_branch(:,I1);
gg_train_out=[U1_train_out(:,I1);V1_train_out(:,I1)];
%
ff_test=UV0_test_branch(:,I2);
param_test=PARAM_test_branch(:,I2);
gg_test_out=[U1_test_out(:,I2);V1_test_out(:,I2)];
%
ff_val=ff_test;
param_val=param_test;
gg_val_out=gg_test_out;


ff_tot=UV0_train_branch;
%
pod_modes=1;
if pod_modes==1
bias_proj=zeros(82,1); %mean(ff_tot,2);
%Cov=1/(size(ff_val,2)-1)*(ff_val-bias_proj)*(ff_val-bias_proj)';
Cov=1/(size(ff_tot,2)-1)*(ff_tot*ff_tot');
[Vkov,Dkov]=eig(Cov);
...
save('PODmodes3_single.mat','Vkov_proj');
%
...
for Nb=NNb
    ...
    for it=1:iters
        ...
        RandONet=train_RandONet(ff_train,Ytrunk,gg_train_out,Nt,Nb,kmodel,gpu_flag,n_out);
        %
...
folder='figures/';
if parametric==1
fig_base='fig_FHN_0005_0995_multi_par_RandONet_';
else
fig_base='fig_FHN_010_single_par_RandONet_';
end
...
if parametric==1
    ...
    save('POD_RandONet_single_FHN_010','RandONet','flag_single','parametric','simmetry')
end
if pod_modes==1
    save('POD3modes_single','Vkov_proj','bias_proj')
end
end
