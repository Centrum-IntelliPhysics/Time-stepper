...
function net=train_RandONet(ff,yy,G,Nt,Nb,kmodel,gpu_flag,n_out)
...
alphat=2*(4+Nt*1/5)*(2*rand(1,Nt)-1)/DX; %random internal weights of the trunk
...
if flag_branch==0 %RFFN
    alphab=2*Nb^(1/8)*sqrt(2/Nb)*randn(Nb,Nx)/DU.*rand(Nb,Nx); %RFFN random internal weighrs in the branch
...
if (Nb^2*Ns+Ns^2+Nb)<2*100^3 || gpu_flag==0
    tic;
    ...
    tol=min(max(eps(norm(Br_train)*max(size(Br_train)))*1e1,1e-12),1e-5);
    %tol=1e-8;
    invBr=pinv(Br,tol);
