...
%
rng(5)
load('data_00_38_Bratu_chebfun_aligned_shorter_healing2.mat')
save_on=1; %<----------------
%
...
val_epsilon=3.9; %1 or 3.9 (is effectively 3.8 max)  <--------------------
%
if val_epsilon<3.8
    parametric=0;
end
...
%
dt=0.001;
variation_on=0; Ny=51; %variation is basically if one find the map Un+1=Un+f(Un) (so the difference)
...
%
Nt=150; %number of neurons in the trunk hidden layer
        ...
        for it=1:iters
            ...
            if parametric==0
                if flag_single==0
                    ...
                    RandONet=train_RandONet(ff_train(1:end-1,:),yy,G_train,Nt,Nb,kmodel,gpu_flag);
                end
            else
                RandONet=train_RandONet_parametric(ff_train(1:end-1,:),ff_train(end,:),yy,G_train,Nt,Nb,kmodel,gpu_flag);
            end
...
folder='figures/';
if parametric==1
fig_base='fig_Bratu_00_38_multi_par_RandONet_';
else
fig_base='fig_Bratu_10_single_par_RandONet_';
end
...
if save_on==1
if parametric==1
    save('RandONet_parametric_Bratu_00_38','RandONet','flag_single','parametric','simmetry')
else
    save('RandONet_single_Bratu_10','RandONet','flag_single','parametric','simmetry')
end
end