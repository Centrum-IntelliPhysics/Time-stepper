...
function net=train_RandONet_parametric(ff,param,yy,G,Nt,Nb,kmodel,gpu_flag,n_out)
...
if flag_branch==0
    alphab0=2*sqrt(Nx)*sqrt(2/Nb)*randn(Nb,Nx)/DU.*(rand(Nb,Nx)); %RFFN random internal weights in the branch
    alphab1=2*sqrt(Nx)*sqrt(2/Nb)*randn(Nb,Nx)/DU.*(rand(Nb,Nx)); %RFFN random internal weights in the branch
...
%
if (Nb^2*Ns+Ns^2+Nb)<2*1000^3 || gpu_flag==0
    tic;
