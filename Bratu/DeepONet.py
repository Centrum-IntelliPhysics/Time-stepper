...
class DeepONet(nn.Module):
    def __init__(self, branch_layers, trunk_layers, dtype=pt.float32):
        super(DeepONet, self).__init__()

        self.branch_input_size = branch_layers[0]
        self.p = branch_layers[-1]
        assert trunk_layers[-1] == self.p
    ...
    def forward(self, x):
        ...
        output = pt.sum(branch_output[:,:] * trunk_output[:,:], dim=1)
        return output[:,None]